...
Two entry points share `Generator`: `bin/sdk-generate` (standalone CLI) and `src/Laravel/GenerateSdkCommand.php` (`php artisan sdk:generate <consumer>` in a producer API project, configured via `config/sdk-generator.php`, runs the l5-swagger pregenerate hook and the consumer's Pint).
...
- The client uses `#[Singleton]` + `#[Config]` container attributes (needs Laravel ≥ 12.31), no service provider.
